...
void Vysyx_25060170_fishtailcore::prepareClone() const { contextp()->prepareClone(); }
void Vysyx_25060170_fishtailcore::atClone() const {
...
void Vysyx_25060170_fishtailcore___024root__trace_decl_types(VerilatedVcd* tracep);
...
VL_ATTR_COLD static void trace_init(void* voidSelf, VerilatedVcd* tracep, uint32_t code) {
    ...
    Vysyx_25060170_fishtailcore___024root__trace_decl_types(tracep);
...
VL_ATTR_COLD void Vysyx_25060170_fishtailcore::traceBaseModel(VerilatedTraceBaseC* tfp, int levels, int options) {
    (void)levels; (void)options;
    VerilatedVcdC* const stfp = dynamic_cast<VerilatedVcdC*>(tfp);
    if (VL_UNLIKELY(!stfp)) {
        vl_fatal(__FILE__, __LINE__, __FILE__,"'Vysyx_25060170_fishtailcore::trace()' called on non-VerilatedVcdC object;"
            " use --trace-fst with VerilatedFst object, and --trace-vcd with VerilatedVcd object");
    }
    stfp->spTrace()->addModel(this);
    stfp->spTrace()->addInitCb(&trace_init, &(vlSymsp->TOP));
    Vysyx_25060170_fishtailcore___024root__trace_register(&(vlSymsp->TOP), stfp->spTrace());
}
