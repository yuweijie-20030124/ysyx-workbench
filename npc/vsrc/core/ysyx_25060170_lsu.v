...
wire [4:0] byte_shift = {req_byte_sel, 3'b000};   // 0 / 8 / 16 / 24
wire [4:0] half_shift = {req_half_sel[1], 4'b0000}; // 0 / 16
    ...
    if (rst == `ysyx_25060170_RSTABLE) begin
        ...
        case (ls_state)
            ...
            S_LS_WAIT_R: begin
                if (r_hs) begin
                    // 当前版本保持你的原语义：
                    // rresp != OKAY 时，load 返回 0；后续如需异常可在这里扩展
                    rdata_buf <= (arb_lsu_rresp == AXI_RESP_OKAY) ? arb_lsu_rdata : 32'b0;
                    ls_state  <= S_LS_RESP;
                end
            end
...
assign pc_o      = bypass_no_mem ? pc_i      : req_pc;
