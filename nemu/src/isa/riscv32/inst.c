...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  //INSTPAT("0000000 ????? ????? 101 ????? 01100 11", srli   , I, R(rd) = src1 >> BITS(imm, 4, 0)); 
  INSTPAT("0100000 ????? ????? 101 ????? 00100 11", srai   , I, R(rd) = (int32_t)src1 >> BITS(imm , 4 , 0) ); 
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(rd) = Mr(src1 + imm, 1));

  //INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, s->dnpc = (src1 + imm) & (~0x1); R(rd) = s->snpc); 
  ...
  // INSTPAT("??????? ????? ????? 011 ????? 00000 11", fld    , I, R(rd) = Mr(src1 + imm, 8)); 
  // INSTPAT("??????? ????? ????? 010 ????? 00000 11", flw    , I, R(rd) = Mr(src1 + imm, 4)); 
  // INSTPAT("??????? ????? ????? 011 ????? 01000 11", fsd    , I, Mw(src1 + imm, 8, src2));
  // INSTPAT("??????? ????? ????? 010 ????? 00000 11", fsw    , I, Mw(src1 + imm, 4, src2));
